...
/**
 * The shared document shell. Airflow, capacity and combination reports all
 * carry one header and one footer; only the sections between them differ.
 *
 * @param {object} o
 * @param {string} o.title     headline, after the optional client name
 * @param {string} o.verify    the "measured and verified with…" sentence
 * @param {Array}  o.meta      [{l, v}] meta-strip rows; blank values are dropped
 * @param {string} o.footNote  the calibration / honesty note
 */
function reportShell(o){
  const clientName = $('cName').value.trim();
  const clientAddr = $('cAddr').value.trim();
  const co    = o.company || 'Homestar HVAC Solutions';
  const tech  = $('cTech').value.trim() || o.tech || 'Calvin Windsor';
  const phone = o.phone || '';
  const email = o.email || '';

  const meta = [{l:'Service address', v:clientAddr}].concat(o.meta || [])
    .filter(m => m && m.v)
    .map(m => `<div class="meta"><div class="l">${esc(m.l)}</div><div class="v">${esc(m.v)}</div></div>`)
    .join('');
  ...
  <div class="rhead">
    ...
    <div class="rtitle">
      ...
      <h2>${clientName?esc(clientName)+' — ':''}${esc(o.title)}</h2>
      <div class="verify">${esc(o.verify)}</div>
    </div>
    <div class="metarow">${meta}</div>
  </div>`, foot: `
  <div class="rfoot">
    <div class="prep">Prepared by ${esc(tech)}<br><span>${esc(co)}</span></div>
    <div class="fl">${esc(o.footNote)}</div>
  </div>` };
}

const AIRFLOW_FOOTNOTE = 'Measurements captured with a calibrated TEC TrueFlow\u00AE flow grid and DG-8 pressure gauge. Values are field-measured operating parameters at the time of test.';

function airflowShell(before, after){
  return reportShell({
    title: 'Before & After System Report',
    verify: 'System airflow and static pressure were measured and verified with the TrueFlow\u00AE System Airflow and Static Pressure Analysis, before and after the upgrades performed by '
      + (after.company || before.company || 'Homestar HVAC Solutions') + '.',
    meta: [
...
/** The airflow report's own sections, with no shell around them. */
function airflowSections(before, after){
...
function buildReport(before, after){
  const shell = airflowShell(before, after);
  $('sheet').innerHTML = shell.head + airflowSections(before, after) + shell.foot;
}
